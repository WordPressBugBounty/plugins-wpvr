...
function wpvr_block_render($attributes)
{
    ...
    if (isset($postdata['vidid'])) {
        ...
        if (strpos($videourl, 'youtube') > 0 || strpos($videourl, 'youtu') > 0) {
            ...
            $html .= '<div id="' . $random_id . '-permission-request" style="position:absolute; top:0; left:0; width:100%; height:100%; display:none; flex-direction:column; justify-content:center; align-items:center; background-color:rgba(0,0,0,0.7); color:white; text-align:center; border-radius:' . esc_attr( $radius ) . ';">';
            ...
            // Browser warning
            $html .= '<div id="' . $random_id . '-browser-warning" style="position:absolute; top:0; left:0; width:100%; height:100%; display:none; flex-direction:column; justify-content:center; align-items:center; background-color:rgba(0,0,0,0.7); color:white; text-align:center; border-radius:' . esc_attr( $radius ) . ';">';
    ...
    if ($control) {
        if (isset($postdata['customcontrol'])) {
            $custom_control = wpvr_parse_custom_control($postdata['customcontrol']);
            $custom_control = wp_parse_args($custom_control, array(
                'panupSwitch' => 'off',
                'panDownSwitch' => 'off',
                'panLeftSwitch' => 'off',
                'panRightSwitch' => 'off',
    ...
    if (isset($postdata['scene_navigation']) && $postdata['scene_navigation'] === 'on' && 'valid' == $status  && $is_pro) {
        ...
        $html .= '<div id="custom-scene-navigation' . esc_attr( $id ) . '" class="custom-scene-navigation">
              ...
              <div id="custom-scene-navigation-nav' . esc_attr( $id ) . '" class="' . esc_attr( $scene_navigation_nav_class ) . '">
                  <ul></ul>
              </div> 
              ';
    }
    ...
    if ($vrgallery &&  'valid' == $status  && $is_pro ) {
        ...
        $tour_layout = is_array( $postdata['tourLayout'] ?? null )
            ? ( $postdata['tourLayout']['layout'] ?? 'default' )
            : ( $postdata['tourLayout'] ?? 'default' );
        $gallery_layout_class = 'layout1' === $tour_layout
            ? 'wpvr-gallery--modern'
            : 'wpvr-gallery--classic';

        $html .= '<div id="sccontrols' . esc_attr( $id ) . '" class="scene-gallery vrowl-carousel owl-theme ' . esc_attr( $gallery_layout_class ) . '">';
...
//              if(localStorage.getItem("vr_mode") == "off") {
    ...
    if ($vrgallery) {
        ...
        $html .= '
        var owlGallery' . $id . ' = jQuery("#sccontrols' . $id . '").owlCarousel({
            items: 1,
            loop: true,
            nav: false,
            dots: false,
        ...
        jQuery("#pano' . $id . '").on("click", ".wpvr_owl_prev", function() {
            owlGallery' . $id . '.trigger("prev.owl.carousel");
        });
        jQuery("#pano' . $id . '").on("click", ".wpvr_owl_next", function() {
            owlGallery' . $id . '.trigger("next.owl.carousel");
        });
        ';
    }
