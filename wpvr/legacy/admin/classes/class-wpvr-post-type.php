...
class WPVR_Post_Type {
    ...
    public function copy_shortcode_html( $post_id = 0 )
    {
        $shortcode = '[wpvr id="' . intval( $post_id ) . '"]';
        $copy = '<button type="button" class="wpvr-copy-shortcode-listing" data-shortcode="' . esc_attr( $shortcode ) . '" title="Copy shortcode" aria-label="Copy shortcode">
    ...
    public function set_columns($columns) {
        $checkbox = isset($columns['cb']) ? $columns['cb'] : '<input type="checkbox" />';

        // Set/unset post type table columns.
        $columns = array(
            'cb'            => $checkbox,
            'title'         => __('Title', 'wpvr'),
            'thumbnail'     => __('Tour Thumbnail', 'wpvr'),
            'shortcode'     => __('Shortcode', 'wpvr'),
            'type'          => __('Tour Type', 'wpvr'),
    ...
    public function edit_columns($column) {
        ...
        switch ($column) {
            case 'shortcode':
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- SVG icon is hardcoded, not user input
                echo '<div class="wpvr-listing-shortcode"><span class="wpvr-code">[wpvr id="' . esc_attr($post->ID) . '"] </span>' . $this->copy_shortcode_html( $post->ID ) . '</div>';
                break;
