...
class Wpvr_Admin {
    ...
    public function enqueue_scripts() {
        ...
        $admin_script_path = plugin_dir_path(__FILE__) . 'js/wpvr-admin.js';

        if (defined('WPVR_PRO_PLUGIN_DIR_URL') && defined('WPVR_PRO_PLUGIN_DIR_PATH') && $asset_url === WPVR_PRO_PLUGIN_DIR_URL . 'admin/') {
            $admin_script_path = WPVR_PRO_PLUGIN_DIR_PATH . 'admin/js/wpvr-admin.js';
        }
    ...
    public function add_import_button() {
        ...
        $is_pro_available = apply_filters('is_wpvr_pro_active', false) && $status === 'valid';
        ...
        if ($screen && property_exists($screen, 'id') && ($screen->id === 'edit-wpvr_item')) {
            ...
            <script type="text/javascript">
                jQuery(document).ready(function($) {
                    ...
                    var isProUser = <?php echo wp_json_encode($is_pro_available); ?>;
                    ...
                    if (!isProUser) {
                        importButton
                            .addClass('wpvr-import-button--locked')
                            .attr({
                                'aria-disabled': 'true',
                                'aria-haspopup': 'dialog'
                            })
                            .append("<span class='is-pro' aria-hidden='true'>Pro</span>");
                    }
